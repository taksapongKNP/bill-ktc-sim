...
export const PageOne: React.FC<Props> = ({
  ...
  const [comName, setComName] = useState<string>(s_comp);
  const [proName, setProName] = useState<string>(s_titl);
  const [date, setDate] = useState<string>(s_issu);
  const [workDetail, setWorkDetail] = useState<string>(s_deta);
  // const [proName, setComName] = useState<string>(s_imag);
  const [etcDetail, setEtcDetail] = useState<string>(s_rema);
  const [dateStart, setDateStart] = useState<string>(s_star);
        ...
        <Row gutter={{ xs: 8, sm: 16, md: 24, lg: 32 }}>
          <Col className="gutter-row" span={8}>
            <Form.Item label="Company Name" required tooltip="This is a required field">
              <Input placeholder="Company Name" name="comName" onChange={(e)=>{handleComNameChang(e.target.value)}} value={comName} allowClear />
            </Form.Item>
          </Col>
          <Col className="gutter-row" span={8}>
            <Form.Item label="Project Name" required tooltip="This is a required field">
              <Input placeholder="Project Name" name="proName" onChange={(e)=>{handleProNameChang(e.target.value)}} value={proName} allowClear />
            </Form.Item>
          </Col>
          <Col className="gutter-row" span={8}>
